...
<div  class="card mb-3 " style="width: 20.5rem;top: 2px;display: inline-grid; white-space: nowrap;">
    ...
    <div class="w3-card-3"> 
        ...
        <img style="height: 270px; width: 300px;" src="{{asset('storage/' . $molde->image)}}">
    ...
    <div class="container">
        <h5 class="card-title" style="height:35px">
            ...
            <span class="card-title grey-text text-darken-4 truncate" title="{{$molde->nome}}">{{$molde->nome}}</span>
            <p style="margin-top: 10px;">R$ {{$molde->id}} <a style="margin-left:10px;" class="btn  btn-outline-info" href="{{route('produto.show' , $molde->id)}}">Veja mais informações</a> </p>
    </div>
    ...
    <div class="card-body" style="margin-top: 20px;" >
        ...
        <div class="container" style="">
            ...
            <form method="post"action="{{route('favorito.store')}}">
                {{ csrf_field () }}
                <input type="hidden" name="id" value="{{$molde->id }}">

          
                <button class="btn-large col l6 m6 s6 "> Favoritos</button>
                <a style="margin-left:50px; "href="{{route('adicionar',  $molde->id)}}" class="btn btn-outline-info">  
